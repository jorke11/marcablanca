...
class AdministradorModel extends MY_Model {
    ...
    public function datosUsuarios($datos = NULL, $user = NULL, $where = NULL) {
        ...
        if ($this->session->userdata("client_id") != '') {
            if ($where == null) {
                $where = "WHERE usuarios.idmarca=" . $this->session->userdata("client_id");
            } else {
                $where = " AND usuarios.idmarca=" . $this->session->userdata("client_id");
            }
        }
...
//        return  $res->result_array();
    ...
    public function datosUsuarios2($datos = NULL, $user = NULL, $where = NULL) {
        $where = ($where == NULL) ? '' : 'WHERE usuarios.' . $where;
        if ($this->session->userdata("client_id") != '') {
            if ($where == null) {
                $where = "WHERE usuarios.idmarca=" . $this->session->userdata("client_id");
