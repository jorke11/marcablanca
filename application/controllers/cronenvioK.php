...
class cronenvioK extends MY_Controller {
    ...
    public function index() {  
        ...
        else
        {
            ...
            if($data["cron"][0]["estado"] == 1)
            {
                ...
                $this->registraLog('cronenvioK', 'cronenvioProcess', $nuevo);
            }    
            else
            {
                $this->cronenvioIn($data["cron"][0]["id"]);
                    ...
                    while($contador < 55)
                    {
                         ...
                         */
                        $where = "nomenclatura = 'K' ";
                        $data["canal"] = $this->AdministradorModel->obtenerCampos('canales', 'id', $where);

                        $where = "estado = '2' AND (idcanal is null OR idcanal = ".$data["canal"][0]["id"].") "
                        ...
                        if ($tam == 0)
                        {
                            echo $contador." - NO HAY MENSAJES PARA EL CANAL L<br>";
                        ...
                        else 
                        {
                            ...
                            $data["smpp"] = $this->AdministradorModel->obtenerCampos('canales', 'host,port,usr,password,systemtype,nomenclatura,sourceaddress', $where);
                            ...
                            if($conectado == 1)
                            {
                                for ($i = 0; $i < $tam; $i++) {
                                    if ($data["pendientes"][$i]["idcanal"]) {
                                        ...
                                        $data["canal"] = $this->AdministradorModel->obtenerCampos('canales', '*', $where);
                                    }
                                    //2. enviar mensaje SMPP
                                    echo $i . " - enviando mensaje " . $data["pendientes"][$i]["numero"] . " " . $data["pendientes"][$i]["mensaje"] . " <br>";
                                    $datos = 0;
                                    //$resultado = $this->enviarsms($datos, $data["pendientes"][$i]["numero"], $data["pendientes"][$i]["mensaje"]);
                                    $resultado = $smpp->Send($data["pendientes"][$i]["numero"], $data["pendientes"][$i]["mensaje"]);
    ...
    function actualizar($id,$idbase,$idcarrie,$numero,$mensaje,$nota,$carrie,$cargue,$canal,$fcargue) {
            ...
            $registro[0][13] = $id;
            $separador = ';';
            $this->crearPlano('/var/www/html/marcablanca/planos/'.$data["ultimo"][0]["archusuario"] , $titulo, $registro, $separador);
            $this->crearPlano('/var/www/html/marcablanca/planos.c/'.$data["ultimo"][0]["archmes"] , $titulo, $registro, $separador);
            $this->crearPlano('/var/www/html/marcablanca/planos/'.$data["ultimo"][0]["archconsolidado"] ,$titulo, $registro, $separador);
    }
    ...
    function cronenvioIn($id){
        ...
        $this->registraLog('cronenvioA', 'cronenvioIn', '+++++');
    }
    ...
    function cronenvioOut($id){
        ...
        $this->registraLog('cronenvioA', 'cronenvioOut', '-----');
    }
